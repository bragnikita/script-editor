...
export const getCursorXY = (input: HTMLTextAreaElement, selectionPoint: number, offset: number) => {
    const {
        offsetLeft: inputX,
        offsetTop: inputY,
    } = input
    ...
    Object.getOwnPropertyNames(copyStyle).forEach((prop: string) => {

    });
    div.style.cssText = copyStyle.cssText;
